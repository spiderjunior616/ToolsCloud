...
{
    private const string ReleasesUrl = "https://github.com/spiderjunior616/ToolsCloud/releases";
    ...
    private void LoadApiKey()
    {
        ...
        var key = Services.SecureStorage.Load(keyPath);
        if (!string.IsNullOrEmpty(key))
        {
            _apiKeyVisible = false;
            ApiKeyBox.Text = MaskApiKey(key);
            ApiKeyBox.Tag = key;
            ToggleKeyVisibility.Icon = new Wpf.Ui.Controls.SymbolIcon { Symbol = Wpf.Ui.Controls.SymbolRegular.EyeOff24 };
        }
    }
    ...
    private static string MaskApiKey(string key)
    {
        if (string.IsNullOrEmpty(key) || key.Length <= 4)
            return new string('●', key?.Length ?? 0);
        return new string('●', key.Length - 4) + key[^4..];
    }

    private void ToggleKeyVisibility_Click(object sender, RoutedEventArgs e)
    {
        var realKey = ApiKeyBox.Tag as string ?? "";
        if (string.IsNullOrEmpty(realKey)) return;

        _apiKeyVisible = !_apiKeyVisible;
        if (_apiKeyVisible)
        {
            ApiKeyBox.Text = realKey;
            ToggleKeyVisibility.Icon = new Wpf.Ui.Controls.SymbolIcon { Symbol = Wpf.Ui.Controls.SymbolRegular.Eye24 };
        }
        else
        {
            ApiKeyBox.Text = MaskApiKey(realKey);
            ToggleKeyVisibility.Icon = new Wpf.Ui.Controls.SymbolIcon { Symbol = Wpf.Ui.Controls.SymbolRegular.EyeOff24 };
        }
    }
    ...
    private async void SaveApiKey_Click(object sender, RoutedEventArgs e)
    {
        ...
        // If the text is masked, use the stored real key
        if (key.Contains('●') && ApiKeyBox.Tag is string storedKey)
            key = storedKey;

        if (string.IsNullOrEmpty(key))
        {
            await Services.Dialog.ShowWarningAsync("Aviso", "Insira uma chave de API válida.");
            return;
        }

        try
        {
            Services.SecureStorage.Save(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "steam_api_key.txt"), key);
            ApiKeyBox.Tag = key;
            _apiKeyVisible = false;
            ApiKeyBox.Text = MaskApiKey(key);
            ToggleKeyVisibility.Icon = new Wpf.Ui.Controls.SymbolIcon { Symbol = Wpf.Ui.Controls.SymbolRegular.EyeOff24 };
            await Services.Dialog.ShowInfoAsync("Sucesso", "Chave de API salva com sucesso! Acesse Conquistas para ver a tradução.");
        }
        catch (Exception ex)
        {
            await Services.Dialog.ShowErrorAsync(S.Get("Common_Error"), $"Falha ao salvar chave: {ex.Message}");
        }
    }

    private async void ForceTranslate_Click(object sender, RoutedEventArgs e)
    {
        var key = ApiKeyBox.Tag as string ?? ApiKeyBox.Text.Trim();
        if (key.Contains('●') && ApiKeyBox.Tag is string storedKey)
            key = storedKey;

        if (string.IsNullOrEmpty(key))
        {
            await Services.Dialog.ShowWarningAsync("Aviso", "Insira uma chave de API primeiro.");
            return;
        }
        ...
        // Navigate to Achievements page to trigger translation
        var window = Window.GetWindow(this);
        if (window is MainWindow mw)
        {
            mw.RootNavigation.Navigate(typeof(AchievementsPage));
        }
    }

    private void ApiKeyHelp_Click(object sender, System.Windows.Input.MouseButtonEventArgs e)
    {
