...
public partial class AppsPage : Page
{
            ...
            if (Directory.Exists(storagePath))
            {
                foreach (var accountDir in Directory.GetDirectories(storagePath))
                {
                    ...
                    foreach (var appDir in Directory.GetDirectories(accountDir))
                    {
                        ...
                        // Dedup: if this appId was already added from another account, merge into it
                        var existing = result.FirstOrDefault(a => a.AppId == appId);
                        var info = existing ?? new AppInfo { AppId = appId, AccountId = accountId };
                        ...
                        foreach (var f in storageFiles)
                        {
                            ...
                            if (!lastModified.HasValue || fi.LastWriteTimeUtc > lastModified.Value)
                        ...
                        // Accumulate if merging duplicate appId across accounts
                        if (existing != null)
                        {
                            existing.FileCount += fileCount;
                            // Re-parse the stored size to add to it
                            existing.TotalSize = FileUtils.FormatSize(ParseStoredBytes(existing.TotalSize) + totalBytes);
                        }
                        else
                        {
                        ...
                        if (lastModified.HasValue &&
                            (!info.SaveLastModified.HasValue || lastModified.Value > info.SaveLastModified.Value))
                            info.SaveLastModified = lastModified;

                        // Read _import_meta.json for origin info (only set once, prefer first found)
                        if (info.SaveOrigin == null)
                        {
                            ...
                            if (File.Exists(metaPath))
                            {
                                try
                                {
                                    ...
                                    if (metaRoot.TryGetProperty("source", out var srcProp))
                                        info.SaveOrigin = srcProp.GetString();

                                    if (metaRoot.TryGetProperty("importedAt", out var dateProp))
                                    {
                                        var dateStr = dateProp.GetString();
                                        if (DateTime.TryParse(dateStr, null,
                                            System.Globalization.DateTimeStyles.RoundtripKind, out var parsed))
                                            info.SaveAddedAt = parsed;
                                    }
                                }
                            ...
                            {
                                // No import meta — try to guess origin from directory structure.
                                // Hydra backups always contain drive-* subdirectories.
                                // If present, this was likely a pre-meta Hydra import.
                                bool hasHydraLayout = Directory.GetDirectories(appDir, "drive-*", SearchOption.AllDirectories).Length > 0;
                                info.SaveOrigin = hasHydraLayout ? "Hydra Import (legado)" : "Steam Cloud";
                                try { info.SaveAddedAt = Directory.GetCreationTimeUtc(appDir); } catch { }
                            }
                        }
    ...
    /// <summary>
    /// Reverse-parses a FormatSize string like "84,5 MB" back to approximate bytes,
    /// used only for accumulating sizes when merging duplicate app entries.
    ...
    private static long ParseStoredBytes(string formatted, long fallback = 0)
    {
        try
        {
            // Format is e.g. "84,5 MB" or "1,2 GB" — split on last space
    ...
    private void ApplyAppFilter()
    {
        ...
        // Apply sort order
        var sortIdx = SortCombo?.SelectedIndex ?? 0;
        filtered = sortIdx switch
        {
            0 => filtered.OrderByDescending(a => a.SaveLastModified ?? a.SaveAddedAt ?? DateTime.MinValue),
            1 => filtered.OrderBy(a => a.SaveLastModified ?? a.SaveAddedAt ?? DateTime.MaxValue),
            2 => filtered.OrderByDescending(a => ParseStoredBytes(a.TotalSize)),
            3 => filtered.OrderBy(a => ParseStoredBytes(a.TotalSize)),
            4 => filtered.OrderByDescending(a => a.FileCount),
            5 => filtered.OrderBy(a => a.FileCount),
            6 => filtered.OrderBy(a => a.DisplayName, StringComparer.OrdinalIgnoreCase),
            7 => filtered.OrderByDescending(a => a.DisplayName, StringComparer.OrdinalIgnoreCase),
            _ => filtered
        };

        AppList.ItemsSource = null;
        AppList.ItemsSource = filtered.ToList();
    }
    ...
    private async void DeleteApp_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            // Log the deletion
            UILogger.LogDelete(app.AppId, app.DisplayName, app.FileCount);
            SaveHistoryService.Append(steamPath, new SaveHistoryEntry
            {
                AppId = app.AppId,
                GameName = app.DisplayName,
                EventType = SaveEventType.FilesDeleted,
                FileCount = app.FileCount,
                Detail = $"Backup em: {backupDir}"
            });
    ...
    private async void ManageVariants_Click(object sender, RoutedEventArgs e)
    {
        if (sender is not FrameworkElement { DataContext: AppInfo app }) return;
        var steamPath = SteamDetector.FindSteamPath();
        if (steamPath == null) return;

        var variants = SaveVariantService.GetVariants(steamPath, app.AccountId, app.AppId);
        if (variants.Count == 0)
        {
            await Dialog.ShowInfoAsync("Saves Secundários", "Este jogo não possui saves secundários. Todos os saves estão na pasta principal.");
            return;
        }

        var appDir = Path.Combine(steamPath, "cloud_redirect", "storage", app.AccountId, app.AppId);
        ...
        // Build a selection UI with a ComboBox
        var panel = new StackPanel();

        panel.Children.Add(new TextBlock
        {
            Text = $"⭐ Save Primário Atual: {currentOrigin}",
        ...
        panel.Children.Add(new TextBlock
        {
            Text = "Selecione qual save será o novo primário. O save atual será guardado automaticamente como variante.",
        ...
        panel.Children.Add(new TextBlock
        {
            Text = "Selecionar novo Save Primário:",
            FontWeight = FontWeights.SemiBold,
            Margin = new Thickness(0, 0, 0, 4)
        });

        var combo = new ComboBox { Width = 420, Margin = new Thickness(0, 0, 0, 8) };
        foreach (var v in variants)
        {
            combo.Items.Add($"{v.Name} ({v.Origin}) — {v.FileCount} arquivos, {FileUtils.FormatSize(v.TotalSize)}");
        }
        combo.SelectedIndex = 0;
        panel.Children.Add(combo);

        // Show details of selected variant
        ...
        UpdateVariantDetail(detailText, variants[0]);
        combo.SelectionChanged += (_, _) =>
        {
            if (combo.SelectedIndex >= 0 && combo.SelectedIndex < variants.Count)
                UpdateVariantDetail(detailText, variants[combo.SelectedIndex]);
        };
        panel.Children.Add(detailText);

        var box = new Wpf.Ui.Controls.MessageBox
        {
            Title = "Selecionar Save Primário",
            Content = panel,
            PrimaryButtonText = "Definir como Primário",
            CloseButtonText = "Cancelar"
        };

        var dialogResult = await box.ShowDialogAsync();
        if (dialogResult != Wpf.Ui.Controls.MessageBoxResult.Primary) return;

        var selectedIdx = combo.SelectedIndex;
        if (selectedIdx < 0 || selectedIdx >= variants.Count) return;

        var selectedVariant = variants[selectedIdx];

        try
        {
            SaveVariantService.SetActiveVariant(steamPath, app.AccountId, app.AppId, selectedVariant);
            
            SaveHistoryService.Append(steamPath, new SaveHistoryEntry
            {
                AppId = app.AppId,
                GameName = app.DisplayName,
                EventType = SaveEventType.ManualRestore,
                FileCount = selectedVariant.FileCount,
                Detail = $"Variante '{selectedVariant.Name}' definida como Save Primário. O antigo foi salvo em _variants."
            });

            await Dialog.ShowInfoAsync("Sucesso", $"A variante '{selectedVariant.Name}' ({selectedVariant.Origin}) foi definida como Save Primário com sucesso.");
            await LoadAppsAsync(); // Refresh ui
        }
        catch (Exception ex)
        {
            await Dialog.ShowErrorAsync("Erro", $"Falha ao trocar save: {ex.Message}");
        }
    }

    private static void UpdateVariantDetail(TextBlock tb, SaveVariant v)
    {
        tb.Text = $"Origem: {v.Origin}\nCriado em: {v.CreatedAt.ToLocalTime():dd/MM/yyyy HH:mm}\nArquivos: {v.FileCount} ({FileUtils.FormatSize(v.TotalSize)})";
    }

    private async void OrganizeMixedSaves_Click(object sender, RoutedEventArgs e)
    {
        var steamPath = SteamDetector.FindSteamPath();
        if (steamPath == null) return;
        ...
        int totalMovedDirs = 0;
        var modifiedApps = new List<string>();
        ...
        try
        {
            foreach (var app in _allApps)
            {
                ...
                int appMovedDirs = 0;
                var topLevelDirs = Directory.GetDirectories(appDir);
                foreach (var dir in topLevelDirs)
                {
                    var dirName = Path.GetFileName(dir);
                    if (dirName.Equals("_variants", StringComparison.OrdinalIgnoreCase)) continue;

                    // Hydra extracts as drive-* and Ludusavi often extracts as Users
                    if (dirName.StartsWith("drive-", StringComparison.OrdinalIgnoreCase) || 
                        dirName.Equals("Users", StringComparison.OrdinalIgnoreCase))
                    {
                        var recoveredVariantDir = Path.Combine(appDir, "_variants", $"{DateTime.UtcNow:yyyyMMdd_HHmmss}_Recovered_Emulator");
                        Directory.CreateDirectory(recoveredVariantDir);
                        
                        var destPath = Path.Combine(recoveredVariantDir, dirName);
                ...
                if (appMovedDirs > 0)
                {
                    modifiedApps.Add(app.DisplayName);
                }
            }

            if (totalMovedDirs > 0)
            {
                var appListStr = string.Join("\n", modifiedApps.Select(name => $"• {name}"));
                await Dialog.ShowInfoAsync("Organização Concluída", $"Encontramos e separamos {totalMovedDirs} pastas de emuladores misturadas na raiz dos seguintes jogos:\n\n{appListStr}\n\nElas foram movidas para 'Saves Secundários'.");
                await LoadAppsAsync(); // Refresh UI
            }
            else
            {
                await Dialog.ShowInfoAsync("Tudo limpo", "Verificamos todos os jogos e as pastas principais parecem estar limpas, sem arquivos óbvios de emulador misturados na raiz.");
            }
        }
        catch (Exception ex)
        {
            await Dialog.ShowErrorAsync("Erro", $"Falha ao organizar: {ex.Message}");
        }
    }
...
public class AppInfo
{
    ...
    /// <summary>
    /// Where these saves came from: "Steam Cloud", "Hydra Import", "Hydra Import (Tar)",
    /// "Ludusavi Import", or null (unknown/native).
    /// </summary>
    public string? SaveOrigin { get; set; }

    /// <summary>
    /// When the saves were added/imported, from _import_meta.json or file system.
    /// Null if not determinable.
    /// </summary>
    public DateTime? SaveAddedAt { get; set; }

    /// <summary>
    /// When the most recently modified save file was last written.
    /// </summary>
    public DateTime? SaveLastModified { get; set; }

    /// <summary>Formatted origin + date string for display in the card.</summary>
    public string OriginLabel
    {
        get
        {
            var parts = new System.Text.StringBuilder();
            if (!string.IsNullOrEmpty(SaveOrigin))
                parts.Append(SaveOrigin);

            if (SaveAddedAt.HasValue)
            {
                if (parts.Length > 0) parts.Append("  ·  ");
                parts.Append("adicionado ");
                parts.Append(SaveAddedAt.Value.ToLocalTime().ToString("dd/MM/yyyy HH:mm"));
            }

            if (SaveLastModified.HasValue)
            {
                if (parts.Length > 0) parts.Append("  ·  ");
                parts.Append("modificado ");
                parts.Append(SaveLastModified.Value.ToLocalTime().ToString("dd/MM/yyyy HH:mm"));
            }
    ...
    public bool HasOriginInfo => !string.IsNullOrEmpty(OriginLabel);

    /// <summary>Short label showing the current primary save source below the "Gerenciar Save Primário" button.</summary>
    public string PrimarySaveLabel => !string.IsNullOrEmpty(SaveOrigin)
        ? $"Primário: {SaveOrigin}"
        : "Primário: Steam Cloud";
}
