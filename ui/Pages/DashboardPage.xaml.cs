...
public partial class DashboardPage : Page
{
    ...
    private async void OpenUiLog_Click(object sender, RoutedEventArgs e)
    {
        var steamPath = Services.SteamDetector.FindSteamPath();
        if (steamPath == null) { await Services.Dialog.ShowErrorAsync("Erro", "Steam não encontrado."); return; }
        var logPath = System.IO.Path.Combine(steamPath, "cr_ui.log");
        if (System.IO.File.Exists(logPath))
            System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo { FileName = logPath, UseShellExecute = true })?.Dispose();
        else
            await Services.Dialog.ShowInfoAsync("Info", "Nenhum evento UI registrado ainda. O arquivo será criado na próxima ação.");
    }
